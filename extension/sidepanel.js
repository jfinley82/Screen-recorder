...
function showState(name) {
  ...
  if (name !== "recording") document.body.classList.remove("collapsed");
}
...
document.getElementById("btn-collapse").addEventListener("click", () => {
  ...
  document.getElementById("btn-collapse").title = collapsed ? "Expand panel" : "Collapse panel";
});
...
async function loadDevices() {
  ...
  if (saved.micId)    document.getElementById("select-mic").value = saved.micId;
  if (saved.cameraId) document.getElementById("select-camera").value = saved.cameraId;
...
document.getElementById("btn-login").addEventListener("click", async () => {
  ...
  errorEl.classList.add("hidden"); btn.disabled = true; btn.textContent = "Logging in…";
  try {
...
document.getElementById("btn-logout").addEventListener("click", async () => {
  await chrome.storage.local.clear(); showState("login");
});

// ── Recording ─────────────────────────────────────────────────────────────────
let recorder = null, chunks = [], animHandle = null;
...
document.getElementById("btn-start").addEventListener("click", async () => {
  ...
  micMuted = false; micTrackRef = null;
  ...
  try {
    ...
    if (currentMode === "desktop+camera" && cameraId) {
      try {
        ...
        startRecording(new MediaStream([...stream.getVideoTracks(), ...(micTrack ? [micTrack] : [])]), quality,
          () => cancelAnimationFrame(animHandle));
        return;
      } catch { /* fall through */ }
    }

    startRecording(new MediaStream([...screenStream.getVideoTracks(), ...(micTrack ? [micTrack] : [])]), quality);
  } catch (err) {
...
function buildComposite(screenStream, cameraStream) {
  ...
  const sv = document.createElement("video"),  cv = document.createElement("video");
  ...
  function draw() {
    ...
    ctx.closePath(); ctx.clip(); ctx.drawImage(cv, cx, cy, CAM_W, CAM_H); ctx.restore();
...
function startRecording(stream, quality, onStop) {
  ...
  recorder = new MediaRecorder(stream, { mimeType, videoBitsPerSecond: quality === "1080" ? 5_000_000 : 2_500_000 });
  recorder.ondataavailable = (e) => { if (e.data.size > 0) chunks.push(e.data); };
  recorder.onstop = () => { stream.getTracks().forEach((t) => t.stop()); onStop?.(); uploadRecording(new Blob(chunks, { type: mimeType })); };
  recorder.start(1000);
  showState("recording");
  startTimer();
  stream.getVideoTracks()[0]?.addEventListener("ended", () => { if (recorder?.state !== "inactive") recorder.stop(); stopTimer(); });
}
...
document.getElementById("btn-again").addEventListener("click", () => {
  collapsed = false; showState("ready");
});
...
async function uploadRecording(blob) {
  ...
  try {
    ...
    chrome.notifications.create("", {
      ...
      title: "Recording saved!", message: "Your recording is in your library.",
    });
  } catch (err) {
    alert(`Upload failed: ${err.message}`); showState("ready");
  }
}
...
async function goReady() {
  ...
  document.getElementById("link-lib-done").href = `${APP_URL}/library`;
  showState("ready"); setMode("desktop"); await loadDevices();
}

async function init() {
  STATES.forEach((s) => document.getElementById(`state-${s}`).style.display = "none");
